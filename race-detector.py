...
prediction_face = model.predict(img)
prediction_proba = race_model.predict(img)

print(len(prediction_face))
print(np.argmax(prediction_face))

for i in range(6):
    result = "{race}: {probability:.6f}"
    print(result.format(race=races[i], probability=prediction_proba[0][i]))
